...
<div id="app">
  ...
  $telephone = new Telephone($db);
    $enclos = $telephone->getEnclos(4);
    $telephone->getAnimalFromEnclosure($enclos);
    
    foreach ($enclos->getAnimals() as $key => $renard) {
  echo '
   <a href="fullscreen.php?id='.$renard->getId().'&enclos='. $enclos->getId().'">
    <div class="item animal'.$key.'">
      <div class="col2">
        <p class="carte">Nom : '. $renard->getName(). ' </p>
        <p class="carte">Poid : '. $renard->getPoid(). ' </p>
        <p class="carte">Taille : '. $renard->getTaille(). ' </p>
      </div>
      <img class="renard" src="../img/objets/renard.png"> 
    </div>
    </a>
  ';
}
